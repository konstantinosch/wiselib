...
namespace wiselib
{
	...
	public:
		...
		void send( node_id_t destination, size_t len, block_data_t *data, message_id_t msg_id )
		{
			...
			radio().send( destination, message.serial_size(), (uint8_t*) &message );
		}
		// -----------------------------------------------------------------------
	...
	//great care XXXXXXXXXXXXXXXX
		...
		// -----------------------------------------------------------------------
		void process_query( PLTT_Agent a )
		{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
			debug().debug( "PLTT_Passive %x: process_query - entering\n", self.get_node().get_id() );
#endif
			for ( PLTT_TraceListIterator traces_iterator = traces.begin(); traces_iterator != traces.end(); ++traces_iterator )
			{
				if ( a.get_target_id() == traces_iterator->get_target_id() )
				{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
					debug().debug( "PLTT_Passive %x: process_query - found target_id %x - further lookup\n", self.get_node().get_id(), a.get_target_id() );
#endif
					if ( ( traces_iterator->get_intensity() * 100 ) / a.get_max_intensity() >= intensity_detection_threshold )
					{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
						debug().debug( "PLTT_Passive %x: process_query - found target_id %x - target is in the area, switching to report mode!", self.get_node().get_id(), a.get_target_id() );
#endif
						a.switch_dest();
						process_report( a );
					}
					...
					{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
						debug().debug( "PLTT_Passive %x: process_query - found target_id %x - target is not in the area - further lookup!", self.get_node().get_id(), a.get_target_id() );
#endif
						if ( traces_iterator->get_parent().get_id() != 0x0 )
						{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
					debug().debug( "PLTT_Passive %x: process_query - found target_id %x - target is not in the area - further lookup! - found origin and forwarding\n", self.get_node().get_id(), a.get_target_id() );
#endif
							block_data_t buff[ReliableRadio::MAX_MESSAGE_LENGTH];
							a.serialize( buff );
							reliable_radio().send( traces_iterator->get_parent().get_id(), a.serial_size(), buff );
						}
						else
						{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
						debug().debug( "PLTT_Passive %x: process_query - found target_id %x - target is not in the area - further lookup! - non-coherent trace (old detection inhibited...) forwarding to random recipient\n", self.get_node().get_id(), a.get_target_id() );
#endif
							if ( neighbors.size() != 0 )
							{
								...
								block_data_t buff[ReliableRadio::MAX_MESSAGE_LENGTH];
								a.serialize( buff );
								reliable_radio().send( recipient, a.serial_size(), buff );
							}
							else
							{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
								debug().debug( "PLTT_Passive %x: process_query - did not find tracker_id %x - forwarding to random recipient - FAILED, empty neighbor list\n", self.get_node().get_id(), a.get_target_id() );
#endif
							}
						}
					}
				...
				{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
					debug().debug( "PLTT_Passive %x: process_query - did not find target_id %x - forwarding to random recipient\n", self.get_node().get_id(), a.get_target_id() );
#endif
					if ( neighbors.size() != 0 )
					{
						node_id_t recipient = neighbors.at( rand()() % neighbors.size() ).get_node().get_id();
					...
					{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
					debug().debug( "PLTT_Passive %x: process_query - did not find target_id %x - forwarding to random recipient - FAILED, empty neighbor list\n", self.get_node().get_id(), a.get_target_id() );
#endif
					}
				}
			}
		...
		// -----------------------------------------------------------------------
		void process_report( PLTT_Agent a )
		{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
			debug().debug( "PLTT_Passive %x: process_report - entering\n", self.get_node().get_id() );
#endif
			for ( PLTT_TraceListIterator traces_iterator = traces.begin(); traces_iterator != traces.end(); ++traces_iterator )
			{
				if ( a.get_tracker_id() == traces_iterator->get_target_id() )
				{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
					debug().debug( "PLTT_Passive %x: process_report - found tracker_id %x - further lookup\n", self.get_node().get_id(),  a.get_tracker_id());
#endif
					if ( ( traces_iterator->get_intensity() * 100 ) / a.get_max_intensity() >= intensity_detection_threshold )
					{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
					debug().debug( "PLTT_Passive %x: process_query - found tracker_id %x - tracker is in the area report back!\n", self.get_node().get_id(), a.get_target_id() );
#endif
						block_data_t buff[ReliableRadio::MAX_MESSAGE_LENGTH];
						a.serialize( buff );
						reliable_radio().send( a.get_tracker_id(), a.serial_size(), buff );
					}
					else
					{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
					debug().debug( "PLTT_Passive %x: process_report - found tracker_id %x - tracker is not in the area - further lookup!\n", self.get_node().get_id(), a.get_target_id() );
#endif
						if ( traces_iterator->get_parent().get_id() != 0x0 )
						{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
					debug().debug( "PLTT_Passive %x: process_report - found tracker_id %x - tracker is not in the area - further lookup! - found origin and forwarding\n", self.get_node().get_id(), a.get_target_id() );
#endif
							block_data_t buff[ReliableRadio::MAX_MESSAGE_LENGTH];
							a.serialize( buff );
							reliable_radio().send( traces_iterator->get_parent().get_id(), a.serial_size(), buff );
						...
						{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
						debug().debug( "PLTT_Passive %x: process_report - found tracker_id %x - tracker is not in the area - further lookup! - non-coherent trace (old detection inhibited...) forwarding to random recipient\n", self.get_node().get_id(), a.get_target_id() );
#endif
							if ( neighbors.size() != 0 )
							{
								node_id_t recipient = neighbors.at( rand()() % neighbors.size() ).get_node().get_id();
							...
							{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
								debug().debug( "PLTT_Passive %x: process_report - did not find tracker_id %x - forwarding to random recipient - FAILED, empty neighbor list\n", self.get_node().get_id(), a.get_target_id() );
#endif
							}
						}
					}
				...
				{
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
					debug().debug( "PLTT_Passive %x: process_report - did not find tracker_id %x - forwarding to random recipient\n", self.get_node().get_id(),  a.get_tracker_id());
#endif
					if ( neighbors.size() != 0 )
					{
						node_id_t recipient = neighbors.at( rand()() % neighbors.size() ).get_node().get_id();
...
#ifdef PLTT_PASSIVE_DEBUG_NEIGHBORHOOD_DISCOVERY
			debug().debug( "PLTT_Passive %x: process_report - exiting\n", self.get_node().get_id() );
#endif
		}
		// -----------------------------------------------------------------------
		...
		void proactive_inhibition_daemon( void* userdata = NULL )
		{
			...
			if ( self.get_node_target_list()->size() > 0 )
			{
				...
				power.set_dB( transmission_power_dB);
				radio().set_power( power );
				radio().send( Radio::BROADCAST_ADDRESS, self.serial_size(), buff );
			}
