...
namespace wiselib
{
	...
	enum error_codes
	{
        ...
        uint8_t get_nb_receive_stability( node_id_t id )
        {
            ...
            for ( iterator_t it = neighborhood.begin(); it != neighborhood.end(); ++it )
            {
                if ( it->id == id )
                {
                    uint32_t millis = (uint32_t) clock().milliseconds(clock().time()) + clock().seconds(clock().time()) * 1000 - (uint32_t) clock().milliseconds(it->first_beacon) - (uint32_t) clock().seconds(it->first_beacon) * 1000;
        ...
        // --------------------------------------------------------------------
        uint8_t get_nb_stability_send( node_id_t id )
        {
            for ( iterator_t it = neighborhood.begin(); it != neighborhood.end(); ++it )
            {
                if ( it->id == id )
        ...
        // --------------------------------------------------------------------
        uint8_t get_nb_avg_LQI_send( node_id_t id )
        {
            for ( iterator_t it = neighborhood.begin(); it != neighborhood.end(); ++it)
            {
                if ( it->id == id )
                {
                    return it->avg_LQI_send;
                }
            }
        }
        // --------------------------------------------------------------------
        uint8_t get_nb_avg_LQI_rec(node_id_t id)
        {
            for (iterator_t it = neighborhood.begin(); it != neighborhood.end(); ++it)
            {
                if (it->id == id)
                {
                    return it->avg_LQI_rec;
                }
            }
        }
        // --------------------------------------------------------------------
        void init( Radio& radio, Clock& clock, Timer& timer, Debug& debug, uint16_t bp = 1000, uint16_t tp = 3000, uint8_t minlqi = 255, uint8_t maxlqi = 255, uint8_t st = 80 )
        {
            radio_ = &radio;
            clock_ = &clock;
            timer_ = &timer;
            ...
            beacon_period = bp;
            timeout_period = tp;
            min_lqi_threshold = minlqi;
            max_lqi_threshold = maxlqi;
        ...
        // --------------------------------------------------------------------
        void set_timeout_period( uint16_t timeout_pd )
        {
        ...
        uint8_t reg_event_callback( uint8_t alg_id, uint8_t events_flag, T *obj_pnt )
        {
            for ( reg_alg_iterator_t it = registered_apps.begin(); it != registered_apps.end(); it++ )
            {
                if ( it->alg_id == alg_id )
                {
                    it->event_notifier_callback = event_notifier_delegate_t::template from_method<T, TMethod > ( obj_pnt );
            ...
            entry.alg_id = alg_id;
            entry.size = 0;
            entry.event_notifier_callback = event_notifier_delegate_t::template from_method<T, TMethod > ( obj_pnt );
        ...
        void register_debug_callback( uint8_t flags )
        {
            if ( flags == 0 )
            {
                flags = self_t::NEW_NB | self_t::NEW_NB_BIDI | self_t::DROPPED_NB | self_t::NEW_PAYLOAD_BIDI | self_t::LOST_NB_BIDI;
            }
    ...
    private:
...
#endif
                received_nb( from, ex, recvmsg );
                received_bidi_nb( from, ex, recvmsg );
            }
        };
		// --------------------------------------------------------------------
        void received_nb( node_id_t from, ExData ex, EchoMsg_t recvmsg )
        {
        	bool known = false;
            ...
            for ( ; it != neighborhood.end(); ++it )
            {
            	if ( it->id == from )
            	{
                    it->avg_LQI_rec = ( (it->avg_LQI_rec * it->total_beacons) + ex.link_metric() ) / (it->total_beacons + 1);
                    it->total_beacons++;
                    if ( !it->active )
                    {
                        break;
                    ...
                    if ( !it->stable )
                    {
                        if ( ( ex.link_metric() > min_lqi_threshold ) ||
                        	 ( get_nb_receive_stability( from )> max_stability_threshold ) ||
                        	 ( node_stability > it->stability ) )
                        {
                            return;
                        }
                    }
                    ...
                    if ( ( ex.link_metric() > min_lqi_threshold ) &&
                    	 ( get_nb_receive_stability( from ) > max_stability_threshold ) &&
                    	 ( node_stability > it->stability ) &&
                         ( it->beacons_in_row == ECHO_TIMES_ACC_NEARBY ) && ( !it->stable ) )
                       {
                            it->stable = true;
                            it->first_beacon = clock().time();
                            it->total_beacons = 1;
            ...
            if ( !known )
            {
                ...
                if ( neighborhood.size() < neighborhood.max_size() )
                {
                    if ( it == neighborhood.end() )
                    {
                        ...
                        new_nb_entry.avg_LQI_rec = ex.link_metric();
                        new_nb_entry.avg_LQI_send = 0;
            ...
            for (int i = 0; i < recvmsg->get_pg_payloads_num(); i++)
            {
            	for (reg_alg_iterator_t it = registered_apps.begin(); it != registered_apps.end(); it++)
            	{
                    if ( (it->alg_id == *alg_pl)    && (it->event_notifier_callback != 0) )
                    {
                        if ( (it->events_flag & (uint8_t) NEW_PAYLOAD) == (uint8_t) NEW_PAYLOAD)
                        {
                            it->event_notifier_callback(NEW_PAYLOAD, from, *(alg_pl + 1), alg_pl + 2);
                        }
						//else if ( ( (it->events_flag & (uint8_t) NEW_PAYLOAD_BIDI ) == (uint8_t) NEW_PAYLOAD_BIDI ) && is_neighbor_bidi(from) )
						//{
						//	it->event_notifier_callback( NEW_PAYLOAD_BIDI, from, *( alg_pl + 1 ), alg_pl + 2 );
                        //}
                    }
                }
                alg_pl += *(alg_pl + 1) + 2;
        ...
        // --------------------------------------------------------------------
        void received_bidi_nb( node_id_t from, ExData ex, EchoMsg_t recvmsg )
        {
            for ( iterator_t it = neighborhood.begin(); it != neighborhood.end(); ++it )
             {
                 ...
                 if ( it->id == from )
                 {
                     ...
                     while ( nb_size_bytes != bytes_read )
                     {
                         ...
                         it->avg_LQI_send = read<OsModel, block_data_t, uint8_t > ( recvmsg->payload() + bytes_read );
                         bytes_read += sizeof (uint8_t);
                         it->stab_send = read<OsModel, block_data_t, uint8_t > ( recvmsg->payload() + bytes_read );
                         bytes_read += sizeof (uint8_t);
					...
					if ( ( it->stability > max_stability_threshold) && ( node_stability > max_stability_threshold ) )
                     {
//??
                     }
                 }
                 ...
                 if ( contains_my_id )
                 {
                 	if ( !it->bidi )
                 	{
                 		if ( ( it->stab_send > stability_threshold ) &&
                 			 ( get_nb_receive_stability( from ) > stability_threshold ) )
                 		{
                 			it->bidi = true;
        ...
        void debug_callback(uint8_t event, node_id_t from, uint8_t len,
            ...
            if (self_t::NEW_NB == event) {
...
#else
                debug_->debug("NB;%x;%x", from, radio_->id(), get_nb_receive_stability(from));
#endif
                ...
                debug_->debug(
                        ...
                        stable_nb_size(), node_stability);
#else
                debug_->debug("NBB;%x;%x", from, radio_->id(), get_nb_stability_send(from));
#endif
        ...
        void cleanup_nearby() {
            ...
            for (iterator_t
                ...
                //                    if (it->stable) {
                //
                //                        uint8_t rec_stab = get_nb_receive_stability(it->id);
                //                        if (rec_stab <= stability_threshold) {
                //                            debug().debug("dropping %x rec_stab: %d <= %d", it->id, rec_stab, stability_threshold);
...
#else
                if (it->stable) {
                    msg->add_nb_entry(it->id, it->avg_LQI_rec, get_nb_receive_stability(it->id));
                }
#endif
            }
