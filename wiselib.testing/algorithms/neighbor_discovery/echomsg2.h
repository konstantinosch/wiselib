...
//            write<OsModel, block_data_t, uint8_t > (buffer + PG_NUM, (uint8_t)0);
        ...
        {
        	write<OsModel, block_data_t, node_id_t >(buffer + buffer[NBS_NUM] + PAYLOAD_POS + 1, nb_id);
        	buffer[NBS_NUM] += sizeof(node_id_t);
            buffer[PAYLOAD_POS] += sizeof(node_id_t);
        };

        void add_nb_entry( node_id_t nb_id, uint8_t avg_LQI, uint8_t nb_rec_stab )
        {
        	write<OsModel, block_data_t, node_id_t >( buffer + buffer[NBS_NUM] + PAYLOAD_POS + 1, nb_id );
        	buffer[PAYLOAD_POS] += sizeof( node_id_t );
        	buffer[NBS_NUM] += sizeof( node_id_t );
        	write<OsModel, block_data_t, uint8_t >( buffer + buffer[NBS_NUM] + PAYLOAD_POS + 1, avg_LQI );
        	buffer[PAYLOAD_POS] += sizeof( uint8_t );
        	buffer[NBS_NUM] += sizeof( uint8_t );
        	write<OsModel, block_data_t, uint8_t >( buffer + buffer[NBS_NUM] + PAYLOAD_POS + 1, nb_rec_stab );
        	buffer[PAYLOAD_POS] += sizeof( uint8_t );
        	buffer[NBS_NUM] += sizeof( uint8_t );
        };
...
//            buffer[NBS_NUM] += sizeof(node_id_t);
//            buffer[PAYLOAD_POS] += sizeof(node_id_t);
//            write<OsModel, block_data_t, uint8_t >(buffer + buffer[NBS_NUM] + PAYLOAD_POS + 1, nb_st);
//            buffer[NBS_NUM] += sizeof(uint8_t);
//            buffer[PAYLOAD_POS] += sizeof(uint8_t);
//
//        };
...
//            buffer[NBS_NUM] += sizeof(uint8_t);
//            buffer[PAYLOAD_POS] += sizeof(uint8_t);
//            write<OsModel, block_data_t, uint16_t >(buffer + buffer[NBS_NUM] + PAYLOAD_POS + 1, nd_st);
